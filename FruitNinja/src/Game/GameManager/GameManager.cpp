...
namespace Engine {
	...
	void GameManager::OnUpdate()
	{
		...
		for (auto GameObject : m_GameObjects)
		{
			//if (glm::vec2(m_Player->GetScreenCoordinates()) == glm::vec2(GameObject->GetScreenCoordinates()))
			//{
			//	ENGINE_TRACE("Collision Occured!")
			//}
			ENGINE_TRACE("Player at {0}, {1}", GameObject->GetScreenCoordinates().x, GameObject->GetScreenCoordinates().y);
